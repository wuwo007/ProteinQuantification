...
bool tree::birth(std::ofstream& log, size_t nid, size_t v, size_t c, double ml, double mr)
{
	tree_p np = getptr(nid);
	if (np == 0) 
	{
		cout << "error in birth: bottom node not found\n";
		log << "error in birth: bottom node not found\n";
		return false; //did not find note with that nid
	}
	if (np->l)
	{
		cout << "error in birth: found node has children\n";
		log << "error in birth: found node has children\n";
...
bool tree::death(std::ofstream& log, size_t nid, double mu)
{
	tree_p nb = getptr(nid);
	if (nb == 0)
	{
		cout << "error in death, nid invalid\n";
		log << "error in death, nid invalid\n";
		return false;
	}
	if (nb->isnog()) 
	{
	...
	else 
	{
		...
		log << "error in death, node is not a nog node\n";
		return false;
	}
}
//--------------------
...
//uses recursion down
void tree::pr(std::ofstream& log, bool pc) const
{
	size_t d = depth();
	size_t id = nid();
