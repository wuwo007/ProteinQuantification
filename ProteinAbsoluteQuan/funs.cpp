...
void getsuff(tree& x, tree::tree_cp nx, size_t v, size_t c, xinfo& xi, dinfo& di, sinfo& sl, sinfo& sr)
{
	...
	for (size_t i = 0; i<di.n; i++) 
	{
		xx = di.x + i*di.p;
		if (nx == x.bn(xx, xi)) 
		{ //does the bottom node = xx's bottom node
			y = di.y[i];
			if (xx[v] < xi[v][c]) 
			{
				sl.n++;
...
void drmu(tree& t, xinfo& xi, dinfo& di, pinfo& pi, RNG& gen)
{
	...
	for (tree::npv::size_type i = 0; i != bnv.size(); i++) 
	{
		...
		ybar = sv[i].sy / sv[i].n;
		temp = gen.normal();
		bnv[i]->setm(b*ybar / (a + b) + temp / sqrt(a + b));
	}
...
//--------------------------------------------------
//make xinfo = cutpoints（只使用了x的每一列的最小值和最大值，按照nc的数目进行了分格，存储在了xi中）
void makexinfo(size_t p, size_t n, double *x, xinfo& xi, size_t nc)
{
	...
	for (size_t i = 0; i<p; i++) {
		for (size_t j = 0; j<n; j++) {
			xx = *(x + p*j + i);
			if (xx < minx[i]) minx[i] = xx;      //没有赋值，直接应用？
...
bool Normalize(double *p, int m, int n)            //using ZScore； 
{
	if (n < 1 || m < 3) 
		return false;

	double *meanXY = new double[n ];
	double *sdeXY = new double[n ];
	for (int i = 0; i < n ; i++)
		meanXY[i] = 0.0;
	for (int i = 0; i < m; i++)            
		for (int j = 0; j < n ; j++)
	...
	for (int i = 0; i < m; i++)
		for (int j = 0; j < n ; j++)
			sdeXY[j] += (*(p + i*n + j) - meanXY[j])*(*(p + i*n + j) - meanXY[j]);
	for (int j = 0; j < n ; j++)
	...
	for (int i = 0; i < m; i++)
		for (int j = 0; j < n ; j++)
			*(p + i*n + j) = (*(p + i*n + j) - meanXY[j]) / sdeXY[j];
	//second , no centralization
...
bool Normalize(double *p,int n)            //using ZScore； 
{
	if (n < 1 )
		return false;
