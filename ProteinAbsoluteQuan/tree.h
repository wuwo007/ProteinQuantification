...
public:
...
#else
	friend bool bd(std::ofstream& log, tree& x, xinfo& xi, dinfo& di, pinfo& pi, RNG& gen);
#endif
	...
	void pr(std::ofstream& log, bool pc = true) const; //to screen, pc is "print children"
	//birth death using nid----------
	bool birth(std::ofstream& log, size_t nid, size_t v, size_t c, double ml, double mr);
	bool death(std::ofstream& log, size_t nid, double mu);
